...
调试面板中的“清除数据库数据”除了管理员身份外，还会额外要求输入清库密钥。  
该密钥由 `dbManager` 云函数环境变量 `DB_CLEAR_KEY` 控制，只有 `clearCollections` 操作会校验它。

请在 `dbManager` 云函数环境变量中新增：

```bash
DB_CLEAR_KEY=请替换为另一段高强度随机字符串
```

建议：
- 不要与 `BOOTSTRAP_ADMIN_KEY` 复用同一个值
- 建议单独由少数维护人员保管
- 如果当前环境不希望允许清库，可以不配置 `DB_CLEAR_KEY`，此时后端会直接拒绝清库
- 清库时现在会经过：管理员校验 -> 两次确认 -> 输入 `DB_CLEAR_KEY` -> 后端校验，四层保护后才会执行
