...
Page({
  data: {
    ...
    quickActions: [
      ...
      { icon: '📕', label: '护照借用', color: '#D97706', bg: '#FEF3C7', implemented: true, featureKey: 'passport_application' }
    ],
    announcements: [],
    todaySchedules: [],
  ...
  handleQuickAction(e) {
    ...
    if (label === '每周菜单') {
      ...
      // 统一权限检查
      this.checkAndNavigate('passport_application', '/pages/office/passport/passport', '护照借用')
    } else {
      utils.showToast({
        title: '功能开发中，敬请期待',
        icon: 'none'
